...
export default {
  data: {
    modalCustomId: 'addProjectModal',
  },

  execute: async (interaction: ModalSubmitInteraction) => {
    const projectName = interaction.fields.getTextInputValue('projectIdInput')
    const logger = new Logger(projectName)
    const project = new Project()
    const reply = await interaction.reply('Project creation requested.')
